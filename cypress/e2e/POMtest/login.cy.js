...
describe('Auth: Login user with different ways', () => {
  ...
  it('Happy Path scenario using POM Locators', () => {
    ...
    cy.fixture('user').then((user) => {
      ...
      LoginLocators.locators.userName.type(user.user2.username).wait(2000);
      LoginLocators.locators.password.type(user.user2.password).wait(2000);
      LoginLocators.locators.submit.click().wait(2000);
    });
    // let's call our custom command to verify the text
    cy.textExists('You logged into a secure area!');
    auth.logout();
